...
void init_gdt() {
  ...
  uint32_t gdt_kernel_segments_offset[NUM_OF_KERNEL_SEGMENTS];
  ...
  for (uint32_t i = 0, j = 8; i < NUM_OF_KERNEL_SEGMENTS; i++) {
    gdt_kernel_segments_offset[i] = j;
    j += 8;
  }
  gdt_flush((uint32_t)&gdt_ptr, (uint32_t)&gdt_kernel_segments_offset);
}
